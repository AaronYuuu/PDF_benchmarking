...
def process_all_vision_info(messages, examples=None):
    ...
    examples_batch = examples if is_batch_examples else ([examples] if examples is not None else [])
    
    # Ensure examples batch matches messages batch if provided
    if examples and len(examples_batch) != len(messages_batch): # type: ignore
        if not is_batch and len(examples_batch) == 1: # type: ignore
    ...
    for i, message_group in enumerate(messages_batch):
        ...
        if examples and i < len(examples_batch): # type: ignore
            input_example_images = extract_example_images(examples_batch[i]) # type: ignore
